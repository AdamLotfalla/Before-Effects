    ...
    Layer::Layer(QWidget* parent, path *p, int* frameWidth) : QWidget(parent)
    {
        ...
        LboundFrame_ = relatedPath_->layerStartFrame_;
        RBoundFrame_ = relatedPath_->layerEndFrame_;
    ...
    void Layer::mouseMoveEvent(QMouseEvent *event)
    {
        if(holding_){
            ...
            if(drawMode_ == DrawMode::keyframe){
                ...
                else if(event->pos().x() >= offset_ + relatedPath_->layerStartFrame_ * *frameWidth_ && event->pos().x() <= offset_ + relatedPath_->layerEndFrame_ * *frameWidth_ && event->pos().y() <= layerHeight_){
    ...
    void Layer::shiftKeyframeLayer(qreal dist)
    {
        ...
        relatedPath_->layerStartFrame_ += frameShifts;
        relatedPath_->layerEndFrame_ += frameShifts;
    ...
    void Layer::paintEvent(QPaintEvent *event)
    {
        ...
        if(drawMode_ == DrawMode::hierarchy){
            ...
            painter.drawRoundedRect(relatedPath_->layerStartFrame_ * *frameWidth_, 0, (relatedPath_->layerEndFrame_ - relatedPath_->layerStartFrame_) * *frameWidth_, 4000, 2, 2); // for layers
            painter.setBrush(QBrush(color_));
            painter.drawRoundedRect(relatedPath_->layerStartFrame_ * *frameWidth_, 0, (relatedPath_->layerEndFrame_ - relatedPath_->layerStartFrame_) * *frameWidth_, layerHeight_, 2, 2);
            ...
            if(relatedPath_->isSelected()){
                ...
                QPainterPath Rect;
                Rect.addRoundedRect(relatedPath_->layerStartFrame_ * *frameWidth_, 0, (relatedPath_->layerEndFrame_ - relatedPath_->layerStartFrame_) * *frameWidth_, layerHeight_, 2, 2);
                painter.setClipPath(Rect);
                float lineSpacing = 30.0;
                int NLines = width() / lineSpacing + 1;
                while(NLines--){
                    painter.drawLine(NLines * lineSpacing + relatedPath_->layerStartFrame_ * *frameWidth_ + 20, -10, NLines * lineSpacing + relatedPath_->layerStartFrame_ * *frameWidth_, keyframeLayerHeight_ + 10);
                }
            ...
            painter.drawRect(relatedPath_->layerStartFrame_ * *frameWidth_, 0, std::max(LboundFrame_ - relatedPath_->layerStartFrame_, 0) * *frameWidth_, height());
            painter.drawRect(RBoundFrame_ * *frameWidth_, 0, std::max(relatedPath_->layerEndFrame_ - RBoundFrame_, 0) * *frameWidth_, height());
            if(!relatedPath_->visible_){
                painter.drawRect(relatedPath_->layerStartFrame_ * *frameWidth_, 0, (relatedPath_->layerEndFrame_ - relatedPath_->layerStartFrame_) * *frameWidth_, height());
            }
